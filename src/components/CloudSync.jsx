...
export default function CloudSync() {
  ...
  const [authMode, setAuthMode] = useState(null); // null | 'signin' | 'signup' | 'magic'
  ...
  const handleSignIn = useCallback(async () => {
    ...
    if (result.error) {
      ...
      if (result.user) {
        ...
        if (applied) {
          setMessage({ type: 'success', text: 'Data synced from cloud. Reload to apply.' });
        }
      }
    }
  }, [email, password]);
  ...
  // Not configured — show setup info
  if (!configured) {
    return (
  ...
  return (
    <div style={{ ...S.card, padding: '14px', marginBottom: 12 }}>
      ...
      <div style={{ fontSize: 12, color: T.t2, fontFamily: T.fm, lineHeight: 1.6, marginBottom: 12 }}>
        Sign in to back up your data to the cloud and sync across devices. Your data stays on your device — cloud is an optional backup.
      </div>
      ...
      {!authMode ? (
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            {authMode === 'signin' && (
              <button onClick={handleSignIn} disabled={loading || !email} style={{ ...S.logBtn, flex: 1, padding: '10px', textAlign: 'center', fontSize: 11, opacity: loading ? 0.5 : 1 }}>
                {loading ? 'Signing in...' : 'Sign In'}
              </button>
            )}
            {authMode === 'signup' && (
              <button onClick={handleSignUp} disabled={loading || !email || !password} style={{ ...S.logBtn, flex: 1, padding: '10px', textAlign: 'center', fontSize: 11, opacity: loading ? 0.5 : 1 }}>
                {loading ? 'Creating...' : 'Create Account'}
              </button>
            )}
            {authMode === 'magic' && (
              <button onClick={handleMagicLink} disabled={loading || !email} style={{ ...S.logBtn, flex: 1, padding: '10px', textAlign: 'center', fontSize: 11, opacity: loading ? 0.5 : 1 }}>
                {loading ? 'Sending...' : 'Send Magic Link'}
              </button>
            )}
            <button onClick={() => { setAuthMode(null); setMessage(null); setEmail(''); setPassword(''); }} style={{ ...S.newVialBtn, padding: '10px 14px', fontSize: 11 }}>Cancel</button>
          </div>

          {authMode !== 'magic' && (
            <button onClick={() => setAuthMode('magic')} style={{ background: 'none', border: 'none', color: T.t3, fontSize: 10, fontFamily: T.fm, cursor: 'pointer', padding: '4px 0', textDecoration: 'underline' }}>
              Use magic link instead (no password)
            </button>
          )}
          {authMode === 'magic' && (
            <button onClick={() => setAuthMode('signin')} style={{ background: 'none', border: 'none', color: T.t3, fontSize: 10, fontFamily: T.fm, cursor: 'pointer', padding: '4px 0', textDecoration: 'underline' }}>
